...
- **Biased downsampling:** the acceptance probability is a function of the estimated frequency (or kernel density) distribution of the observed activation signal $\theta$ (RNA over DNA).
    * The script [`downsample.py`](downsample.py) uses the empirical histogram-based PDF, where the acceptance probability for a record is $min(1, N/(M*B*p_i))$, where $p_i$ is the observed proportion of records in histogram bin _i_ into which the observed value of $\theta$ falls.
